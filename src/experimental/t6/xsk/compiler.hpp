...
class compiler : public gsc::compiler
{
    ...
    // all locals: arguments + user variables
    std::vector<std::string> thread_vars;
...
private:
    ...
    void emit_thread_locals(const gsc::context_ptr& ctx, const std::vector<std::string>& locals);
    ...
    void emit_expr_call_pointer_type(const gsc::context_ptr& ctx, int args, bool method, bool thread);
    ...
    void emit_expr_call_script_function(const gsc::context_ptr& ctx, const std::string& func, std::string_view file, int args, bool method, bool thread);
    ...
    std::uint8_t variable_stack_index(const gsc::context_ptr& ctx, const gsc::name_ptr& name);
    std::string variable_stack_index_str(const gsc::context_ptr& ctx, const gsc::name_ptr& name);
    ...
    bool is_local_call(const std::string& name);
    bool is_builtin_call(const std::string& name);
    bool is_builtin_func(const std::string& name);
    bool is_builtin_method(const std::string& name);
    bool is_constant_condition(const gsc::expr_ptr& expr);
