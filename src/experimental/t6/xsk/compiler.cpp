...
auto compiler::parse_buffer(const std::string& file, std::vector<std::uint8_t>& data) -> gsc::program_ptr
{
    ...
    if (t6_lex_init(&scanner))
        exit(1);

    YY_BUFFER_STATE yybuffer = t6__scan_buffer(reinterpret_cast<char*>(data.data()), data.size(), scanner);
    ...
    t6__delete_buffer(yybuffer, scanner);
    t6_lex_destroy(scanner);
...
void compiler::emit_include(const gsc::include_ptr& include)
{
    ...
    try
    {
        ...
        if(funcs.size() == 0)
        {
            throw gsc::comp_error(include->loc, "error: include file '" + path + "' defines no functions");
        }
...
void compiler::emit_thread(const gsc::thread_ptr& thread)
{
    ...
    emit_thread_locals(ctx, thread_vars);
...
void compiler::emit_thread_locals(const gsc::context_ptr& ctx, const std::vector<std::string>& locals)
{
    // in t6, all locals (variables + arguments) are created at the start of the thread,
    // in one instruction, and live until the end of the thread
    // if there are no variables at all, CheckClearParams is emitted instead
    if (locals.size())
    {
        emit_opcode(ctx, opcode::OP_SafeCreateLocalVariables, locals);
    }
    else
    {
...
void compiler::emit_stmt_waittill(const gsc::context_ptr& ctx, const gsc::stmt_waittill_ptr& stmt)
{
    ...
    for(const auto& arg : stmt->args->list)
    {
        initialize_variable(ctx, arg.as_name);
        emit_opcode(ctx, opcode::OP_SafeSetWaittillVariableFieldCached, variable_stack_index_str(ctx, arg.as_name));
    }
...
void compiler::emit_stmt_switch(const gsc::context_ptr& ctx, const gsc::stmt_switch_ptr& stmt)
{
    ...
    for(auto i = 0; i < stmt->stmt->stmts.size(); i++)
    {
        ...
        if(entry.as_node->type == gsc::node_t::stmt_case)
        {
            if(has_default)
            {
                throw gsc::comp_error(stmt->loc, "default must be last case");
            }
        ...
        else if(entry.as_node->type == gsc::node_t::stmt_default)
        {
            ...
            ctx->transfer_t6(entry.as_default->ctx);
...
void compiler::emit_expr_assign(const gsc::context_ptr& ctx, const gsc::expr_assign_ptr& expr)	
{
    ...
    else
    {
        ...
        switch(expr->type)
        {
            ...
            case gsc::node_t::expr_assign_mod:           emit_opcode(ctx, opcode::OP_Modulus); break;
            case gsc::node_t::expr_assign_shift_left:    emit_opcode(ctx, opcode::OP_ShiftLeft); break;
            case gsc::node_t::expr_assign_shift_right:   emit_opcode(ctx, opcode::OP_ShiftRight); break;
            ...
            case gsc::node_t::expr_assign_bitwise_exor:  emit_opcode(ctx, opcode::OP_Bit_Xor); break;
...
void compiler::emit_expr_binary(const gsc::context_ptr& ctx, const gsc::expr_binary_ptr& expr)
{
    ...
    switch(expr->type)
    {
        case gsc::node_t::expr_equality:         emit_opcode(ctx, opcode::OP_Equal); break;
        case gsc::node_t::expr_inequality:       emit_opcode(ctx, opcode::OP_NotEqual); break;
        case gsc::node_t::expr_less:             emit_opcode(ctx, opcode::OP_LessThan); break;
        case gsc::node_t::expr_greater:          emit_opcode(ctx, opcode::OP_GreaterThan); break;
        case gsc::node_t::expr_less_equal:       emit_opcode(ctx, opcode::OP_LessThanOrEqualTo); break;
        case gsc::node_t::expr_greater_equal:    emit_opcode(ctx, opcode::OP_GreaterThanOrEqualTo); break;
        ...
        case gsc::node_t::expr_bitwise_exor:     emit_opcode(ctx, opcode::OP_Bit_Xor); break;
        case gsc::node_t::expr_shift_left:       emit_opcode(ctx, opcode::OP_ShiftLeft); break;
        case gsc::node_t::expr_shift_right:      emit_opcode(ctx, opcode::OP_ShiftRight); break;
        ...
        case gsc::node_t::expr_mod:              emit_opcode(ctx, opcode::OP_Modulus); break;
...
void compiler::emit_expr_call_pointer(const gsc::context_ptr& ctx, const gsc::expr_call_ptr& expr)
{
    // no child in t6
    assert(!expr->child);
    // no 'call [[ ]]' statement in t6... no builtin pointer call (?)
    assert(!expr->func.as_pointer->builtin);
    const bool thread = expr->thread;
    const bool method = expr->obj.as_node->type != gsc::node_t::null ? true : false;
    const bool builtin = expr->func.as_pointer->builtin;
    std::uint32_t args = expr->func.as_pointer->args->list.size();

    if (thread && builtin)
        throw gsc::comp_error(expr->loc, "function call cannot have more than 1 type (thread, builtin)");

    if (!thread && !builtin) emit_opcode(ctx, opcode::OP_PreScriptCall);
    ...
    emit_expr_call_pointer_type(ctx, args, method, thread);
}

void compiler::emit_expr_call_pointer_type(const gsc::context_ptr& ctx, int num_args, bool method, bool thread)
{
    if (thread)
    {
        if (method)
        {
            emit_opcode(ctx, opcode::OP_ScriptMethodThreadCallPointer, utils::string::va("%d", num_args));
        }
        else
        {
            emit_opcode(ctx, opcode::OP_ScriptThreadCallPointer, utils::string::va("%d", num_args));
        }
    }
    else
    {
        if (method)
        {
            emit_opcode(ctx, opcode::OP_ScriptMethodCallPointer, utils::string::va("%d", num_args));
        }
        else
        {
            emit_opcode(ctx, opcode::OP_ScriptFunctionCallPointer, utils::string::va("%d", num_args));
        }
    }
}
...
void compiler::emit_expr_call_function(const gsc::context_ptr& ctx, const gsc::expr_call_ptr& expr)
{
    ...
    // if called like e.g. maps\mp\_utility::myfunc(), we know it's an import
    if (file != "") far = true;
    else
    {
        ...
        // not a local, not an import from any #included file - assume it's a builtin
        if (!local && !far)
        {
            builtin = true;
        }
    ...
    if (thread && builtin)
        throw gsc::comp_error(expr->loc, utils::string::va("cannot call builtin function '%s' as a thread", name.c_str()));

    if (!thread && !builtin && !(!method && args == 0)) 
    ...
    if (builtin)
    {
        emit_expr_call_function_builtin(ctx, name, args, method);
    }
    else
    {
        emit_expr_call_script_function(ctx, name, file, args, method, thread);
    }
}

void compiler::emit_expr_call_function_builtin(const gsc::context_ptr& ctx, const std::string& func, int args, bool method)
{
    emit_opcode(ctx, method ? opcode::OP_CallBuiltinMethod : opcode::OP_CallBuiltin, { utils::string::va("%d", args), func });
}

void compiler::emit_expr_call_script_function(const gsc::context_ptr& ctx, const std::string& func, std::string_view file, int num_args, bool method, bool thread)
{
    opcode call_code;
    if (thread)
    {
        if (method)
        {
            call_code = opcode::OP_ScriptMethodThreadCall;
        }
        else
        {
            call_code = opcode::OP_ScriptThreadCall;
        }
    }
    else
    {
        if (method)
        {
            call_code = opcode::OP_ScriptMethodCall;
        }
        else
        {
            call_code = opcode::OP_ScriptFunctionCall;
        }
    }
    std::vector<std::string> instr_data;
    if (!file.empty())
    {
        // far call
        instr_data.push_back(std::string(file));
    }
    instr_data.push_back(func);
    instr_data.push_back(std::to_string(num_args));

    emit_opcode(ctx, call_code, instr_data);
}
...
void compiler::emit_expr_function(const gsc::context_ptr& ctx, const gsc::expr_function_ptr& expr)
{
    ...
    emit_opcode(ctx, opcode::OP_GetFunction, name);
}

// only used when doing "variable = undefined;"
...
void compiler::emit_expr_size(const gsc::context_ptr& ctx, const gsc::expr_size_ptr& expr)
{
    ...
    emit_opcode(ctx, opcode::OP_SizeOf);
}

// set == false when you don't want to just assign an rval,
// but e.g. increment/decrement or clear the variable (when assigning undefined)
...
void compiler::emit_array_variable_ref(const gsc::context_ptr& ctx, const gsc::expr_array_ptr& expr, bool set)
{
    ...
    case gsc::node_t::data_name:
    {
        ...
        emit_opcode(ctx, opcode::OP_EvalLocalArrayRefCached, variable_stack_index_str(ctx, expr->obj.as_name));
        if (set) emit_opcode(ctx, opcode::OP_SetVariableField);
    }
        break;
    case gsc::node_t::expr_call:
        // function_call()[index]
        // not legal in stock GSC, though it seems like it could be useful?
        throw gsc::comp_error(expr->loc, "cannot subscript a function call");
        break;
    default:
...
void compiler::emit_field_variable_ref(const gsc::context_ptr& ctx, const gsc::expr_field_ptr& expr, bool set)
{
    ...
    case gsc::node_t::data_self:
        ...
        if (set) emit_opcode(ctx, opcode::OP_SetVariableField);
        break;
    // e.g. level.players[0].nuked = false
    case gsc::node_t::expr_array:
    ...
    case gsc::node_t::expr_field:
        ...
        break;
    case gsc::node_t::data_name:
        emit_opcode(ctx, opcode::OP_EvalLocalVariableCached, variable_stack_index_str(ctx, expr->obj.as_name));
        emit_opcode(ctx, opcode::OP_CastFieldObject);
        emit_opcode(ctx, opcode::OP_EvalFieldVariableRef, field);
        if(set) emit_opcode(ctx, opcode::OP_SetVariableField);
        break;
    case gsc::node_t::expr_call:
        throw gsc::comp_error(expr->loc, "cannot reference field of a function call expression");
        break;
    default:
...
void compiler::emit_local_variable_ref(const gsc::context_ptr& ctx, const gsc::name_ptr& expr, bool set)
{
    ...
    emit_opcode(ctx, opcode::OP_EvalLocalVariableRefCached, variable_stack_index_str(ctx, expr));
...
// this evals an array element, such as level.players[0]
void compiler::emit_array_variable(const gsc::context_ptr& ctx, const gsc::expr_array_ptr& expr)
{
    ...
    {
        // array expression is a local variable reference
        emit_opcode(ctx, opcode::OP_EvalLocalVariableCached, variable_stack_index_str(ctx, expr->obj.as_name));
    }
    else
    {
        // the array is an arbitrary expression (non name)
        emit_expr(ctx, expr->obj);
    }
    emit_opcode(ctx, opcode::OP_EvalArray);
}

// evaluate object field reference, e.g. print(level.players)
...
void compiler::emit_local_variable(const gsc::context_ptr& ctx, const gsc::name_ptr& expr)
{
    ...
    // is local var
    emit_opcode(ctx, opcode::OP_EvalLocalVariableCached, variable_stack_index_str(ctx, expr));
}
...
void compiler::emit_object(const gsc::context_ptr& ctx, const gsc::expr_ptr& expr)
{
    switch(expr.as_node->type)
    {
        ...
        case gsc::node_t::data_self:
            ...
            break;
        case gsc::node_t::data_name:
            emit_opcode(ctx, opcode::OP_EvalLocalVariableCached, variable_stack_index_str(ctx, expr.as_name));
            emit_opcode(ctx, opcode::OP_CastFieldObject);
            break;
        case gsc::node_t::expr_call:
...
{
    throw gsc::comp_error(animtree->loc, "unsupported");
...
{
    thread_vars.push_back(name);

    const auto ctx_it = std::find_if(ctx->local_vars.cbegin(), ctx->local_vars.cend(),
            [&](const gsc::local_var& v) { return v.name == name; });

    if (ctx_it == ctx->local_vars.cend())
    {
...
std::uint8_t compiler::variable_stack_index(const gsc::context_ptr& ctx, const gsc::name_ptr& name)
{
    for(std::size_t i = 0; i < ctx->local_vars.size(); i++)
    {
...
std::string compiler::variable_stack_index_str(const gsc::context_ptr& ctx, const gsc::name_ptr& name)
{
    return utils::string::va("%d", variable_stack_index(ctx, name));
}
